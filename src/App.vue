  ...
  <v-app id="inspire">
    ...
    <v-toolbar color="indigo" dark fixed app>
      ...
      <v-toolbar-items class="hidden-sm-and-down">
        <!-- <v-btn flat v-bind:to="{ name:'AddMovie'}">Add Moview</v-btn> -->
        <v-btn id="add_movie_link" flat v-bind:to="{ name: 'AddMovie' }"
          v-if="current_user">
          Add Movie
        </v-btn>
        <v-btn id="user_email" flat v-if="current_user">
        {{ current_user.email.lastIndexOf("@baidu.com")>-1?current_user.name:current_user.email }}
        </v-btn>
        <v-btn flat v-bind:to="{ name: 'Register' }" v-if="!current_user" id="register_btn">
          Register
        </v-btn>
        <v-btn flat v-bind:to="{ name: 'Login'}" v-if="!current_user" id="login_btn">
          Login
        </v-btn>
...
export default{
  ...
  methods: {
    listenToEvents() {
      bus.$on('refreshUser', () => {
        // console.log('listenToEvents - refreshUser');
        this.fetchUser();
      });
    },
    async fetchUser() {
